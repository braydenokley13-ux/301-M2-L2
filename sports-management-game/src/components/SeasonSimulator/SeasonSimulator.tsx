...
import { generateDraftClass } from '../../game/draftSystem';
import { generateFreeAgents } from '../../game/freeAgency';
...
const SeasonSimulator: React.FC = () => {
  ...
  const handleStartNewSeason = () => {
    // Transition to offseason draft phase
  ...
  const handleStartPreseason = () => {
    // Reset wins/losses for teams and advance season
    const store = useGameStore.getState();
    const updatedTeams = store.teams.map(t => ({ ...t, wins: 0, losses: 0 }));
    const newProspects = generateDraftClass(60);
    const newFreeAgents = generateFreeAgents(30);
    useGameStore.setState({
      teams: updatedTeams,
      phase: 'preseason',
      currentSeason: store.currentSeason + 1,
      currentWeek: 0,
      standings: {},
      playoffBracket: [],
    ...
    <div className="max-w-7xl mx-auto p-4">
      ...
      <div className="bg-arena-mid rounded-xl p-6 mb-6 border border-gray-700">
        <div className="flex items-center justify-between">
          <div>
            <h2 className="font-heading text-xl font-bold text-white">Season {currentSeason}</h2>
            ...
            <table className="w-full">
              <thead>
                <tr className="border-b border-gray-700 text-left">
                  ...
                  <th className="px-4 py-2 text-gray-400 text-xs font-medium uppercase">MVP</th>
                  <th className="px-4 py-2 text-gray-400 text-xs font-medium uppercase">Fan Approval</th>
                </tr>
              </thead>
              <tbody>
                {seasonResults.map(result => (
                  <tr key={result.season} className="border-b border-gray-800">
                    ...
                    <td className="px-4 py-3 text-gray-300 text-sm">{result.mvp}</td>
